	...
	lastMenu(-1)
{
	...
	scrollBar->Appearance.BorderHover = ui::Colour(230, 230, 100);
	scrollBar->Appearance.BorderActive = ui::Colour(230, 230, 100);
...
void GameView::NotifyNotificationsChanged(GameModel * sender)
{
	...
	for(std::vector<Notification*>::iterator iter = notifications.begin(), end = notifications.end(); iter != end; ++iter)
	{
		...
		tempButton->Appearance.BorderHover = ui::Colour(240, 240, 100);
		tempButton->Appearance.TextHover = ui::Colour(240, 240, 100);
		AddComponent(tempButton);
...
void GameView::OnDraw()
{
	...
	else if(showHud)
	{
		...
		g->drawtext(XRES-16-textWidth, 16, (const char*)sampleInfo.str().c_str(), 255, 0, 255, alpha*0.75f);
		...
		if(showDebug)
		{
			...
			g->drawtext(XRES-16-textWidth, 30, (const char*)sampleInfo.str().c_str(), 255, 0, 255, alpha*0.75f);
		}
	}
	...
	if(showHud && introText < 51)
	{
		...
		g->drawtext(16, 16, (const char*)fpsInfo.str().c_str(), 255, 0, 255, alpha*0.75);
	}
	...
	if(infoTipPresence)
	{
		...
		g->drawtext_outline((XRES-Graphics::textwidth((char*)infoTip.c_str()))/2, (YRES/2)-2, (char*)infoTip.c_str(), 240, 240, 100, infoTipAlpha);
	}

	if(toolTipPresence && toolTipPosition.X!=-1 && toolTipPosition.Y!=-1 && toolTip.length())
	{
		if (toolTipPosition.Y == Size.Y-MENUSIZE-10)
			g->drawtext_outline(toolTipPosition.X, toolTipPosition.Y, (char*)toolTip.c_str(), 240, 240, 100, toolTipPresence>51?255:toolTipPresence*5);
		else
			g->drawtext(toolTipPosition.X, toolTipPosition.Y, (char*)toolTip.c_str(), 240, 240, 100, toolTipPresence>51?255:toolTipPresence*5);
	}

	if(buttonTipShow > 0)
	{
		g->drawtext(16, Size.Y-MENUSIZE-24, (char*)buttonTip.c_str(), 240, 240, 100, buttonTipShow>51?255:buttonTipShow*5);
	}
